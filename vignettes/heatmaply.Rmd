...
It is probably more useful to use scaling. heatmaply supports column
and row scaling using the "scale" parameter.
...
heatmaply(x, Rowv = row_dend, Colv = col_dend, scale = "column")
...
heatmaply(mtcars, margins = c(40, 130),
          colors = heat.colors(100), scale = "column")
```

Or even more customized colors using `scale_fill_gradient_fun`:

```{r}
heatmaply(mtcars, margins = c(40, 130), scale = "column",
